...
ENV WENDY_PLATFORM=nvidia-jetson \
    ...
    COLLIE_CAMERA_SOURCE=broker \
    COLLIE_CAMERA_BROKER_URL=http://127.0.0.1:8098/api/camera/frame.jpg \
    COLLIE_CAMERA_BROKER_TIMEOUT_S=0.75 \
    COLLIE_CAMERA_BROKER_WAIT_MS=400 \
    COLLIE_CAMERA_MAX_SOURCE_AGE_S=0.75 \
    COLLIE_CAMERA_HZ=10 \
    ...
    COLLIE_MATCH_REACQUIRE_TIMEOUT_S=10.0 \
    COLLIE_ARRIVAL_HELLO_ENABLED=0 \
    COLLIE_ARRIVAL_HELLO_SETTLE_S=0.35 \
    COLLIE_ARRIVAL_REST_ENABLED=1 \
    COLLIE_ARRIVAL_REST_DURATION_S=5.0 \
    COLLIE_ARRIVAL_POINTING_ENABLED=0 \
    COLLIE_ARRIVAL_POINTING_LABEL=all \
    COLLIE_ARRIVAL_POINTING_TIMEOUT_S=20.0 \
    COLLIE_POINTING_ENABLED=1 \
    COLLIE_POINTING_POLICY=/app/models/pointing/locked_point_actor.jit \
    COLLIE_POINTING_STATUS_URL=http://127.0.0.1:8096/api/status \
    COLLIE_POINTING_DURATION_S=6.0 \
    COLLIE_POINTING_ACTION_GAIN=1.0 \
    COLLIE_POINTING_MAX_RATE_RAD_S=0.60 \
    COLLIE_POINTING_KP=60.0 \
    COLLIE_POINTING_KD=5.0 \
    COLLIE_POINTING_STANDUP_SECONDS=3.0 \
    COLLIE_POINTING_STANDUP_KP=60.0 \
    COLLIE_POINTING_STANDUP_KD=5.0 \
    COLLIE_POINTING_DIRECT_STANDING_HANDOFF=1 \
    COLLIE_RETURN_HOME_ENABLED=1 \
    COLLIE_RETURN_BACKEND=nav2 \
    COLLIE_NAV2_URL=http://127.0.0.1:8100 \
    COLLIE_NAV2_POLL_PERIOD_S=0.10 \
    COLLIE_NAV2_REQUEST_TIMEOUT_S=2.0 \
    COLLIE_RETURN_ARRIVAL_TOLERANCE_M=0.10 \
    COLLIE_RETURN_HEADING_TOLERANCE_DEG=5.0 \
    ...
    COLLIE_RETURN_CLEARANCE_BACKOFF_M=0.25 \
    COLLIE_RETURN_CLEARANCE_REVERSE_MPS=0.10 \
    COLLIE_RETURN_CLEARANCE_TIMEOUT_S=4.0 \
    ...
    COLLIE_RETURN_POSE_CAPTURE_DURATION_S=0.50 \
    COLLIE_RETURN_POSE_CAPTURE_MAX_DRIFT_M=0.04 \
    COLLIE_RETURN_POSE_CAPTURE_MAX_YAW_DRIFT_DEG=4.0 \
    COLLIE_RETURN_MAX_DISTANCE_M=3.0 \
    COLLIE_RETURN_MAX_ODOMETRY_STEP_M=0.15 \
    COLLIE_RETURN_MAX_ODOMETRY_YAW_STEP_DEG=30.0 \
    COLLIE_MEMORY_CAPTURE_TIMEOUT_S=2.0 \
    COLLIE_MATCH_CONFIRMATIONS=2 \
    COLLIE_APPROACH_MATCH_MISSES=3 \
    COLLIE_APPROACH_REACQUIRE_ATTEMPTS=3 \
    COLLIE_APPROACH_REACQUIRE_TIMEOUT_S=10.0 \
...
COPY models/pointing/locked_point_actor.jit models/pointing/locked_point_actor.jit
